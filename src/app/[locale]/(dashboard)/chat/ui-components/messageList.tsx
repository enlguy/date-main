...
export default function MessageList({
  ...
  useEffect(() => {
    ...
    if (
      ...
      messageListRef.current.scrollHeight - messageListRef.current.scrollTop > 1115
    ) {
  ...
  return (
    <div className="flex max-h-screen flex-col">
      <div
        id="chats-header"
        className="border-navy-200 flex h-16 min-h-16 select-none flex-row items-center border-y"
      >
        <div
          className={`${roboto.className} flex grow flex-row items-center justify-center gap-3 text-base font-medium`}
        >
          ...
          {activeChannel.type == 'direct' && (
            <div className="flex flex-row items-center justify-center gap-3">
              <div className="-space-x-2.0 flex flex-row">
              ...
              1:1 between{' '}
              {groupUsers?.map(
                (member, index) => `${member.name}${groupUsers.length - 1 != index ? ' and ' : ''}`
              )}
            </div>
          )}
        ...
        <div className="flex flex-row">
          ...
          <div className="flex flex-row">
            ...
            <div className="border-navy50 my-2 flex min-w-6 items-center justify-center rounded border bg-neutral-100 px-2 text-xs font-normal">
              {activeChannelPinnedMessage ? '1' : '0'}
            </div>
            <div
              className={`p-3 py-3 ${
                activeChannelPinnedMessage && 'cursor-pointer hover:rounded-md hover:bg-neutral-100'
              } `}
              onClick={() => {
            ...
            >
              <Image src="/icons/pin.svg" alt="Pin" className="" width={24} height={24} priority />
            </div>
          </div>
          <div
            className="cursor-pointer p-3 py-3 hover:rounded-md hover:bg-neutral-100"
      ...
      >
        {messages && messages.length == 0 && (
          <div className="flex h-screen w-full select-none flex-col items-center justify-center gap-4 text-xl">
            <Image
              src="/chat-logo.svg"
              alt="Chat Icon"
              className=""
        ...
        {activeChannelPinnedMessage && !activeChannelPinnedMessage.deleted && (
          <Message
            ...
            quotedMessageSender={
              ...
              (activeChannelPinnedMessage.quotedMessage.userId === currentUser.id
                ? currentUser.name
                : groupUsers?.find(
                    (user) => user.id === activeChannelPinnedMessage.quotedMessage.userId
                  )?.name)
            }
