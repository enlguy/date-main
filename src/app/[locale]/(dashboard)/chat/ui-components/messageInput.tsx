...
export default function MessageInput({
  ...
  return (
    <div
      className={`border-navy200 flex w-full select-none flex-col items-center border-y border-r ${
    ...
    >
      ...
      <div
        className={`flex w-full flex-row items-center ${
          quotedMessage ? 'h-[70px]' : '-mt-[1px] h-[114px]'
        }`}
      >
        <form className={`flex grow`} onSubmit={(e) => handleSend(e)}>
          <input
            className={`mr-1 flex h-[50px] grow rounded-md border border-neutral-300 ${
              quotedMessage ? '' : 'my-8'
            } focus:ring-inputring ml-6 px-6 text-sm outline-none placeholder:text-neutral-500 focus:ring-1`}
            ref={inputRef}
        ...
        {!replyInThread && (
          <div
            className="cursor-pointer hover:rounded-md hover:bg-neutral-100"
        ...
        {!replyInThread && (
          <div
            className="relative cursor-pointer hover:rounded-md hover:bg-neutral-100"
            onClick={() => {
              addAttachment();
            }}
          >
