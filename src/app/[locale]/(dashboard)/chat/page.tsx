...
export default function Page() {
  ...
  const [chatSettingsScreenVisible, setChatSettingsScreenVisible] = useState(false);
  ...
  const [changeChatNameModalVisible, setChangeChatNameModalVisible] = useState(false);
  ...
  const [activeChannelPinnedMessage, setActiveChannelPinnedMessage] = useState<pnMessage | null>(
    null
  );
  const [activeThreadChannel, setActiveThreadChannel] = useState<ThreadChannel | null>(null);
  const [activeThreadMessage, setActiveThreadMessage] = useState<pnMessage | null>(null);
  ...
  async function updateChannelMembershipsForPublic(chat) {
    ...
    chat.currentUser
      ...
      .then(async (membershipResponse) => {
        const currentMemberOfThesePublicChannels = membershipResponse.memberships.map(
          (m) => m.channel
        );
  ...
  async function updateChannelMembershipsForGroups(chat, desiredChannelId = '') {
    if (!chat) return;
    chat.currentUser
      .getMemberships({
      ...
      .then(async (membershipResponse) => {
        const currentMemberOfTheseGroupChannels = membershipResponse.memberships.map(
          (m) => m.channel
        );
  ...
  async function updateChannelMembershipsForDirects(chat, desiredChannelId = '') {
    if (!chat) return;
    chat.currentUser
      .getMemberships({
      ...
      .then(async (membershipResponse) => {
        const currentMemberOfTheseDirectChannels = membershipResponse.memberships.map(
          (m) => m.channel
        );
        //  Look for the desired channel ID
  ...
  useEffect(() => {
    async function init() {
      ...
      setGuidedDemo(
        process.env.NEXT_PUBLIC_GUIDED_DEMO ? process.env.NEXT_PUBLIC_GUIDED_DEMO : null
      );
      ...
      if (!localChat.currentUser.profileUrl) {
        const randomProfileUrl = Math.floor(Math.random() * testData.avatars.length);
      ...
      await localChat.getChannels({ filter: `type == 'public'` }).then(async (channelsResponse) => {
  ...
  useEffect(() => {
    if (chat && publicChannels && publicChannels.length > 0) {
      return Channel.streamUpdatesOn(publicChannels, (channels) => {
        const updatedPublicChannels = publicChannels.map((publicChannel, index) => {
  ...
  useEffect(() => {
    if (chat && privateGroups && privateGroups.length > 0) {
      return Channel.streamUpdatesOn(privateGroups, (channels) => {
        const updatedPrivateGroups = privateGroups.map((privateGroup, index) => {
          if (channels[index].name) {
  ...
  useEffect(() => {
    ...
    const removeMentionsListener = chat.listenForEvents({
      ...
      callback: async (evt) => {
        ...
        showUserMessage(
          'You Were Mentioned:',
          'You have been mentioned in the following message: ' + message?.content.text,
  ...
  */
  ...
  function sendChatEvent(eventType: ChatEventTypes, recipients: User[], payload) {
  ...
  function showUserMessage(title: string, message: string, href: string, type = ToastType.INFO) {
    ...
    return (
      <main>
        <div className="flex h-screen w-full flex-col items-center justify-center">
          ...
          <div className="mb-5 flex animate-spin">
            <Image
          ...
          <div className="select-none text-2xl">{loadMessage}</div>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="overflow-y-hidden overscroll-none">
      <RoomSelector
      ...
      <ChatSettingsScreen
        ...
        activeChannelUsers={
          activeChannel?.type == 'group' && privateGroups
            ? privateGroupsUsers[privateGroups.findIndex((group) => group.id == activeChannel?.id)]
            : activeChannel?.type == 'direct' && directChats
              ? directChatsUsers[
                  directChats.findIndex((dmChannel) => dmChannel.id == activeChannel?.id)
                ]
              : publicChannels
                ? publicChannelsUsers[
                    publicChannels.findIndex((channel) => channel.id == activeChannel?.id)
                  ]
                : []
        }
        buttonAction={async () => {
      ...
      />
      <div className={`${!showEmojiPicker && 'hidden'} absolute bottom-2 left-2 z-50 bg-white`}>
        <Picker
          data={data}
          sheetRows={3}
      ...
      <div
        id="chat-main"
        className={`flex h-screen min-h-screen flex-row overscroll-none  ${
          (roomSelectorVisible ||
      ...
      >
        ...
        <div className="relative w-full bg-white">
          <div
            id="chats-main"
            className="mt-[64px] flex max-h-screen w-full grow flex-col bg-white py-0"
          >
            {creatingNewMessage ? (
              <NewMessageGroup
                chat={chat}
              ...
              <MessageList
                ...
                groupMembership={
                  activeChannel?.type == 'group' && privateGroups && privateGroupsMemberships
                    ? privateGroupsMemberships[
                        privateGroups.findIndex((group) => group.id == activeChannel?.id)
                      ]
                    : activeChannel?.type == 'direct' && directChats && directChatsMemberships
                ...
                messageActionHandler={(action, vars) => messageActionHandler(action, vars)}
                usersHaveChanged={() => {
                  refreshMembersFromServer();
                }}
                updateUnreadMessagesCounts={() => {
            ...
            >
              <MessageInput
                ...
                creatingNewMessage={creatingNewMessage}
                showUserMessage={showUserMessage}
                setEmojiPickerTargetsInput={() => setEmojiPickerTargetsInput(true)}
                setShowEmojiPicker={() => {
                  setTimeout(function () {
